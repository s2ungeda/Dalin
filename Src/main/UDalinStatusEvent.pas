...
//        end;
    ...
    asRecovery :
      begin
        ...
        if not App.Engine.ApiManager.RequestUsdtCandle then
        begin
          App.Log(llError, '', 'Failed RequestUsdtCandle') ;
          ShowMessage('Failed RequestUsdtCandle');
          Exit;
        end;
        // 계좌 생성
        ...
        // 자동주문공통설정 로드
        App.LoadAOConfig;
        // 빗썸 스레드로 오픈 주문 조회 한다.( 종목별 조회를 해야 하기에 )
        //App.Engine.TradeCore.BithActOrdThread.DoResume;
        ...
        // 출금허용 주소 리스트 조회
          if not App.Engine.ApiManager.RequestWithdrawAddress then
          begin
            App.Log(llError, '', 'Failed RequestWithdrawAddress') ;
          end;
...
//          ShowMessage('Failed ConnectAll');
    ...
    asLoad :
      begin
          FrmDalinMain.LoadRegedit;
          App.Engine.FormBroker.Load(ComposeFilePath([App.DataDir, App.Config.DATA_FILE]));
          // dll 화면 로드
          //App.Engine.DllFormLoader.Load(ComposeFilePath([App.DataDir, App.Config.DATA_FILE2]), FrmDalinMain);
          ;
