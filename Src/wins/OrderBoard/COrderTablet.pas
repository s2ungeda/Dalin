...
uses
  Windows, Graphics, Classes, Controls, SysUtils, Math, Dialogs, ExtCtrls, Menus, DateUtils,
    // lemon: common
  UConsts, UTypes, UApiTypes,
    // lemon: data
  UFQN, USymbols, UQuoteBroker,
    // lemon: trade
  UOrders, UAccounts, UDecimalHelper,
    // lemon: imports
  DBoardParams, DBoardPrefs
    // App
  ;
...
type
  ...
  TTabletColumnType = (tcOrder, tcQuote, tcPrice, tcGutter, tcAdded, tcStop, tcFill);
  ...
  public
    Price: Double;
    PriceDesc: String;
    OrderQty: array[TPositionType] of double;
    OrderVar: array[TPositionType] of double;
    Quotes: array[TPositionType] of double;
    QuoteDiffs: array[TPositionType] of double;

    QuotePosition: TPositionType;     // 가격의 잔량 Position ( 잔량 통합시 사용 )

    // -- 선행잔량 관련 데이터
    QTRefCounts: array[TPositionType] of double;
    QTQtys: array[TPositionType] of double;
    // -- IV
//    FIV : Double ;
    // 누적체결
    Fills : double;

    StopQty : array[TPositionType] of double;
    ClearStopQty : array[TPositionType] of double;

    BetweenOrder : boolean;
  end;
  ...
  private
    ...
    FOrderQtySums: Array[TPositionType, TPriceVisibleType] of double ;  // 주문

    FStopQtySums : Array[TPositionType, TPriceVisibleType] of double ;  // 잔량
    FClearStopQtySums : Array[TPositionType, TPriceVisibleType] of double ;  // 잔량

    FQuoteSums: Array[TPositionType] of double ;   // 호가 - 잔량
    ...
    FDescCount: integer;
    FAscCount: Integer;
    FShowForceDist: Boolean;
    FForceDistOnLeft: boolean;
    FFillterForceQty: integer;
    FAccumulOnLeft: boolean;
    FShowAccumulFills: boolean;
    FAccumulHoga: integer;
    ...
    procedure DrawInfoCell(aRect: TRect; stText : String;
      aBkColor, aFontColor: TColor); // 'info' cell
    ...
    procedure DrawHideOwnOrder( aItems:TOrderTableItem; iTop, iBottom: integer );
    ...
    // Property
    function GetOrderSum(aType : TPositionType) : double;
    //
    ...
    procedure SetOpenPosition(dValue : double );
    // MinPrice
    ...
    procedure SetAccmulHoga(const Value: integer);
    ...
    function GetStopSum(aType: TPositionType): double;
    procedure DrawRect3(Rect: TRect; Text: String; BkColor, FontColor: TColor;
      Alignment: TAlignment);
    procedure DeleteDrawItem2(aObj: TObject);
    procedure UpdateAccumulFill(iPrcIndex: integer; bClear: boolean);
  ...
  public
    ...
    property Ready: Boolean read FReady write FReady;
      ...
      // dimension
    //property PriceOrientation : TPriceOrientation read FPriceOrientation write FPriceOrientation;
    property OrientationType : TOrientationType read FOrientationType write SetOrientationType;
    ...
    property ForceDistOnLeft  : boolean read FForceDistOnLeft write FForceDistOnLeft;
    property FillterForceQty  : integer read FFillterForceQty write FFillterForceQty;

    property ShowAccumulFills : boolean read FShowAccumulFills write FShowAccumulFills;
    property AccumulOnLeft    : boolean read FAccumulOnLeft    write FAccumulOnLeft;
    property AccumulHoga      : integer read FAccumulHoga      write SetAccmulHoga;
      // order trace
    ...
    property HideOwnOrderTrace: boolean read FHideOwnOrderTrace write FHideOwnOrderTrace;
      // order
    property OrderSum[aType:TPositionType] : double read GetOrderSum;
    property StopSum[aType:TPositionType] : double read GetStopSum;

    property AvgPrice : Double read FAvgPrice write SetAvgPrice ;
    property EvalPL : Double read FEvalPL write SetEvalPL ;
    property OpenPosition : double read FOpenPosition write SetOpenPosition;
    ...
    property OrderByMouseRB: Boolean read FOrderRightButton write FOrderRightButton ;

    property Selected : Boolean read FSelected  write SetSelected;
    property ShowForceDist : Boolean read FShowForceDist write FShowForceDist;
    property ShowForceSum : Boolean read FShowForceSum write FShowForceSum;
    property ForceSumSec : Integer read FForceSumSec write FForceSumSec;
    ...
    //pricesort
    property PriceSort : boolean read FPriceSort write FPriceSort;
    property PriceHoga : integer read FPriceHoga write FPriceHoga;
...
const
  ...
  QTY_WIDTH = 80 ;        // 호가(잔량)영역
  PRICE_WIDTH = 90;       // 가격 영역
  PRICE_WIDTH_WIDE = 90;  // 호가 통합 이후
  ADDED_WIDTH = 250 ;  // 부가 정보 영역
  //IV_WIDTH = 55 ;     // IV  영역
  STOP_WIDTH = 50 ;     // Stop  영역
  GUT_WIDTH = 5 ;     // Gut 영역
  ACF_WIDTH = 45;     // 누적체결 영역
...
begin
  ...
  FColWidths[tcAdded] := ADDED_WIDTH ;
  FColWidths[tcStop]  := STOP_WIDTH ;
  FColWidths[tcFill]  := ACF_WIDTH ;
  ...
  FColVisibles[tcAdded] := False;
  FColVisibles[tcStop]  := False;
  FColVisibles[tcFill]  := False;

    // Colors
  FTabletColor := clWhite;
  FFrameColor := clNavy;

  FQuoteColor[ptShort, ctBg] := SHORT_BG_COLOR ;
  FQuoteColor[ptShort, ctFont] := $000000 ;
  FQuoteColor[ptLong, ctBg] := LONG_BG_COLOR ;
  FQuoteColor[ptLong, ctFont] := $000000 ;
  FOrderColor[ptShort, ctBg] := SHORT_BG_COLOR ;
  FOrderColor[ptShort, ctFont] := $000000 ;
  FOrderColor[ptLong, ctBg] := LONG_BG_COLOR ;
  FOrderColor[ptLong, ctFont] := $000000 ;

  FStopColor[ptLong, ctBg ] :=  LONG_BG_COLOR;
  FStopColor[ptShort, ctBg ]:=  SHORT_BG_COLOR;
    //
...
begin
  ...
  iTop := 0 ;
  iBottom := DEFAULT_HEIGHT ;
  stText := FloatToStr(StopSum[ptShort]);
  //
  ...
  iTop := 0 ;
  iBottom := DEFAULT_HEIGHT ;
  stText := FloatToStr(StopSum[ptLong]);
  //
...
begin
  ...
  iTop := 0 ;
  iBottom := DEFAULT_HEIGHT ;
  stText := FloatToStr(OrderSum[ptShort]);
  //
  aBkColor := SHORT_BG_COLOR ;
  aFontColor := clBlack;
  //
  ...
  iTop := 0 ;
  iBottom := DEFAULT_HEIGHT ;
  stText := FloatToStr(OrderSum[ptLong]);
  //
  aBkColor := LONG_BG_COLOR ;
  aFontColor := clBlack;
  //
...
begin
{
  comment by 2014.09.15

  if (FSymbol = nil)
     or (FSymbol.Spec = nil)
     or (FSymbol.Spec.Market <> mtOption) then Exit;
  ...
  begin
    ...
    begin
      ...
      DrawRect(Rect(aCol.Left, iTop+1, aCol.Right, iBottom),
               stText,
  ...
  aCopyRect := Rect(aCol.Left, iRectTop + 1, aCol.Right, iRectBottom);
...
procedure TOrderTablet.DrawHideOwnOrder(aItems: TOrderTableItem; iTop,
  iBottom: integer);
begin
...
var
  ...
  begin
    ...
    // 이건 안씀..
  procedure DrawAccumulFill( bDraw : boolean );
  var
    aPosType   : TPositionType;
    dQty       : Double;
    aColor : TColor;
    stTxt  : string;
    bCol : TOrderColumnItem;
  begin

    if bDraw then
    begin
      if FQuote = nil then Exit;
      if FQuote.Symbol.Sales.Last = nil then Exit;
      dQty := FQuote.Symbol.Sales.Last.Volume * FQuote.Symbol.Sales.Last.Side;

      if ((Items[i].Fills > 0) and (FQuote.Symbol.Sales.Last.Side = 1)) or
        (Items[i].Fills < 0) and (FQuote.Symbol.Sales.Last.Side = -1)  then
        Items[i].Fills  := Items[i].Fills + dQty
      else
        Items[i].Fills  := dQty;

      if Items[i].Fills > 0 then
        aColor := clRed
      else
        aColor := clBlue;

      stTxt := Format( '%f', [ abs(Items[i].Fills) ]);
    end
    else begin
      stTxt := '';
    end;

    if AccumulOnLeft then
      aPosType  := ptShort
    else
      aPosType  := ptLong;

    bCol := FCols[tcFill, aPosType];
    DrawCell( bCol, iTop+1, iBottom, clWhite,
              stTxt, aColor, taRightJustify, True);
  end;
...
var
  ...
  begin
    ...
    if aTableItem.Quotes[aPosType] = 0 then begin
      ...
      DrawHideOwnOrder( aTableItem, iTop, iBottom );

    end
    else begin
      DrawCell( FCols[aColType, aPosType], iTop+1, iBottom, aColor,
                //Format('%.0n',[aTableItem.Quotes[aPosType]*1.0]),
                GetVol( aTableItem.Price, aTableItem.Quotes[aPosType]),
...
begin
  ...
  if (FQuote.Symbol.Bids.Count = 0) or (FQuote.Symbol.Asks.Count = 0) then Exit;

  dBid := FQuote.Symbol.Bids[1].Price;
  dBid := TicksFromPrice(FSymbol, dBid, 1);
  dAsk := FQuote.Symbol.Asks[1].Price;
...
var
  aOrderCols, aGutCols, aQtyCols, aFillCols, aStopCols : array[TPositionType] of TOrderColumnItem;
...
begin
  ...
  aFillCols[ptLong]   := GetColumn(tcFill, ptLong);
  aFillCols[ptShort]   := GetColumn(tcFill, ptShort);
  ...
  begin
    ...
    begin
        //매도주문
        if Items[i].OrderQty[ptShort] > 0 then begin
          bigInt.convert(Items[i].OrderQty[ptShort]);
          stText := Format('%.*f', [ bigInt.Precision, Items[i].OrderQty[ptShort] ]);
        end
        else
          stText := '';
        if Items[i].OrderVar[ptShort] > 0 then begin
          bigInt.convert(Items[i].OrderVar[ptShort]);
          stText := stText + Format('+%.*f',[bigInt.Precision, Items[i].OrderVar[ptShort]]);
        end
        else if  Items[i].OrderVar[ptShort] < 0 then begin
          bigInt.convert(Items[i].OrderVar[ptShort]);
          stText := stText + Format('%f',[Items[i].OrderVar[ptShort]]);
        end;

        if Items[i].BetweenOrder then
          aFontColor := clRed
        else
          aFontColor := FOrderColor[ptShort, ctFont];

        DrawRect(
          Rect(aOrderCols[ptShort].Left,iTop+1,aOrderCols[ptShort].Right,iBottom),
            stText, aOrderCols[ptShort].Color ,
            aFontColor , taRightJustify);


        // 매수주문
        if Items[i].OrderQty[ptLong] > 0 then begin
          bigInt.convert( Items[i].OrderQty[ptLong]);
          stText := Format('%.*f',[ bigInt.Precision, Items[i].OrderQty[ptLong]]);
        end
        else stText := '';
        if Items[i].OrderVar[ptLong] > 0 then begin
          bigInt.convert( Items[i].OrderVar[ptLong]);
          stText := stText + Format('+%.*f',[bigInt.Precision, Items[i].OrderVar[ptLong]])
        end
        else if Items[i].OrderVar[ptLong] < 0 then begin
          bigInt.convert( Items[i].OrderVar[ptLong]);
          stText := stText + Format('%.*f',[bigInt.Precision, Items[i].OrderVar[ptLong]]);
        end;
        ...
        if Items[i].BetweenOrder then
          ...
          aFontColor := FOrderColor[ptLong, ctFont];
    ...
    if FColVisibles[tcFill] then
    begin

      if Items[i].Fills <> 0  then
        stText := FloatToStr( abs(Items[i].Fills) )
      else
        stText := '';
      ...
      if (AccumulOnLeft) and  (aFillCols[ptShort] <> nil) then

        DrawRect(
        Rect(aFillCols[ptShort].Left,iTop+1,aFillCols[ptShort].Right,iBottom),
          stText, aFillCols[ptShort].Color,
          aColor , taRightJustify)
      else if ( not AccumulOnLeft ) and ( aFillCols[ptLong] <> nil) then
        DrawRect(
        Rect(aFillCols[ptLong].Left,iTop+1,aFillCols[ptLong].Right,iBottom),
          stText, aFillCols[ptLong].Color,
          aColor , taRightJustify)   ;
...
begin
  ...
  begin
    FOrderQtySums[ptLong,pvDown] := FOrderQtySums[ptLong,pvDown] + Items[i].OrderQty[ptLong];
    FOrderQtySums[ptShort,pvDown]:= FOrderQtySums[ptShort,pvDown] + Items[i].OrderQty[ptShort];

    FStopQtySums[ptLong, pvDown] := FStopQtySums[ptLong, pvDown] + Items[i].StopQty[ptLong];
    FStopQtySums[ptShort, pvDown]:= FStopQtySums[ptShort, pvDown] + Items[i].StopQty[ptShort];

    FClearStopQtySums[ptLong, pvDown] := FClearStopQtySums[ptLong, pvDown] + Items[i].ClearStopQty[ptLong];
    FClearStopQtySums[ptShort, pvDown]:= FClearStopQtySums[ptShort, pvDown]+ Items[i].ClearStopQty[ptShort];
...
//    inc(FClearStopQtySums[ptShort, pvDown], Items[i].ClearStopQty[ptShort]);
  ...
  for i := FEndIndex downto FStartIndex do
  begin
    FOrderQtySums[ptLong, pvVisible ] := FOrderQtySums[ptLong, pvVisible ] + Items[i].OrderQty[ptLong];
    FOrderQtySums[ptShort, pvVisible ]:= FOrderQtySums[ptShort, pvVisible ] + Items[i].OrderQty[ptShort];

    FStopQtySums[ptLong, pvVisible]   := FStopQtySums[ptLong, pvVisible] + Items[i].StopQty[ptLong];
    FStopQtySums[ptShort, pvVisible]  := FStopQtySums[ptShort, pvVisible] + Items[i].StopQty[ptShort];

    FClearStopQtySums[ptLong, pvVisible]  := FClearStopQtySums[ptLong, pvVisible] + Items[i].ClearStopQty[ptLong];
    FClearStopQtySums[ptShort, pvVisible] := FClearStopQtySums[ptShort, pvVisible] + Items[i].ClearStopQty[ptShort];
...
//    inc(FClearStopQtySums[ptShort, pvVisible], Items[i].ClearStopQty[ptShort]);
  ...
  begin
    FOrderQtySums[ptLong, pvUp] := FOrderQtySums[ptLong, pvUp] + Items[i].OrderQty[ptLong];
    FOrderQtySums[ptShort,pvUp] := FOrderQtySums[ptShort,pvUp] + Items[i].OrderQty[ptShort];

    FStopQtySums[ptLong, pvUp]  := FStopQtySums[ptLong, pvUp] + Items[i].StopQty[ptLong];
    FStopQtySums[ptShort, pvUp] := FStopQtySums[ptShort, pvUp] + Items[i].StopQty[ptShort];

    FClearStopQtySums[ptLong, pvUp] := FClearStopQtySums[ptLong, pvUp] + Items[i].ClearStopQty[ptLong];
    FClearStopQtySums[ptShort, pvUp]:= FClearStopQtySums[ptShort, pvUp] + Items[i].ClearStopQty[ptShort];
        ...
        oqMove : // 정정
          begin
            if ( FromIndex < 0) or ( ToIndex < 0) then
              Continue;
            Items[FromIndex].OrderVar[PositionType] := Items[FromIndex].OrderVar[PositionType] - Qty;
            Items[ToIndex].OrderVar[PositionType] := Items[ToIndex].OrderVar[PositionType] + Qty;
...
//            Inc(Items[ToIndex].OrderVar[PositionType],Qty);
        ...
        oqAdd :
          begin
            if ( FromIndex < 0)  then Continue;
            Items[FromIndex].OrderQty[PositionType] := Items[FromIndex].OrderQty[PositionType] + Qty;
            Items[FromIndex].OrderVar[PositionType] := Items[FromIndex].OrderVar[PositionType] + VarQty;
...
begin
  ...
  begin
    ...
    if Y <= DEFAULT_HEIGHT then
      ...
      // Get Area Type
    if ColType in [tcGutter , tcAdded, tcFill] then
      AreaType := taGutter
...
begin
  ...
  var iLeft, iRight, iMid : integer;

  iLeft  := 0;
  iRight := FTable.Count-1;

  while iLeft <= iRight do
  begin
    iMid  := (iLeft+iRight) div 2;

    p := TOrderTableItem(FTable.Items[iMid]);

    if Abs( p.Price - dPrice) / Max(Abs(p.Price), Abs(dPrice)) < epsilon then
    begin
      Result := iMid;
      Exit;
    end
    else if p.Price < dPrice then
      iLeft := iMid + 1
    else
      iRight := iMid - 1;
  end;
...
function TOrderTablet.GetOrderSum(aType: TPositionType): double;
begin
...
begin
  ...
  FCols[tcGutter, FPositionTypes[stLeft]] := AddColumn('',     tcGutter, FPositionTypes[stLeft]);
    ...
    // column: left 'added' colum
  if FShowForceDist then
    if FForceDistOnLeft then
      FCols[tcAdded, ptShort] := AddColumn('Added', tcAdded, FPositionTypes[stLeft]);

    // column: left 'Accumulation fills' colum
  if FShowAccumulFills then
    if FAccumulOnLeft then
      FCols[tcFill, ptShort] := AddColumn('누적체결', tcFill, FPositionTypes[stLeft]);
  ...
  FCols[tcStop, FPositionTypes[stRight]] := AddColumn('Stop', tcStop, FPositionTypes[stRight]);
    ...
    // column: Right 'Accumulation fills' colum
  if FShowAccumulFills then
    if not FAccumulOnLeft then
      FCols[tcFill, ptLong] := AddColumn('누적체결', tcFill, FPositionTypes[stRight]);

    // column: right 'added' colum
  if FShowForceDist then
    if not FForceDistOnLeft then
      FCols[tcAdded, ptLong] := AddColumn('Added', tcAdded, FPositionTypes[stRight]);
...
begin
  ...
  FTable.Clear;
    ...
    // set high/low limit
  // 15% 로 설정
  FHighLimit := TicksFromPrice(FSymbol, FSymbol.Last,  10000);

  dComp := 0.0001;
  if FSymbol.Spec.ExchangeType = ekBinance then
  begin
    dComp := 1 / power(10, FSymbol.Spec.Precision);
  end;

  FLowLimit  := Max(TicksFromPrice(FSymbol, FSymbol.Last, - 10000), dComp);
  ...
  begin
      // add step
    with FTable.Insert(0) as TOrderTableItem do
    begin
      Price := dPrice;
      PriceDesc := FSymbol.PriceToStr(dPrice);  //Format('%.*n', [FSymbol.Spec.Precision, dPrice]);
    end;
    ...
    dPrice := TicksFromPrice(FSymbol, dPrice, -1);
      ...
      // check lower limit
    if (dPrice < PRICE_EPSILON) or (dPrice < FLowLimit) then Break;
  end;
...
begin
  ...
  case aTabletPoint.AreaType of
    ...
    taGutter:
      if aTabletPoint.ColType = tcFill then
        UpdateAccumulFill( FPriceIndex, true );
    taOrder:
...
begin
  ...
  if (Button = mbRight ) and ( FLastOrdCnl ) and ( not FOrderRightButton ) then
...
//
procedure TOrderTablet.SetAccmulHoga(const Value: integer);
var
  iV : integer;
begin
  iV := Value - 1;
...
procedure TOrderTablet.UpdateAccumulFill( iPrcIndex : integer; bClear : boolean );
begin
end;
...
begin
  ...
  //pricesort
  if (FPriceSort) and (not FScrollLock) then
  begin
    if FPrevPriceIndex = -1 then
      FPrevPriceIndex := FPriceIndex;
    if Abs(FPrevPriceIndex - FPriceIndex) >= FPriceHoga then
    begin
      ScrollToLastPrice;
      FPrevPriceIndex := FPriceIndex;
    end;
...
begin
  ...
  // get indices
    // BID part
  for i := 0 to FQuote.Symbol.Bids.Count - 1 do
  begin
    FQuoteSums[ptLong] := FQuoteSums[ptLong] + FQuote.Symbol.Bids[i].Volume;
    iQuoteIndex := FindIndex(FQuote.Symbol.Bids[i].Price);
    aTableItem := Items[iQuoteIndex];
    if aTableItem <> nil then
    begin
      if i > FAccumulHoga then
        aTableItem.Fills  := 0;

      iNewMin := Min(iNewMin, iQuoteIndex);
      iNewMax := Max(iNewMax, iQuoteIndex);

      SetQuoteField(tcQuote, ptLong, FQuote.Symbol.Bids[i].Volume);
    end;

    //

    FQuoteSums[ptShort] := FQuoteSums[ptShort] + FQuote.Symbol.Asks[i].Volume;
    iQuoteIndex := FindIndex(FQuote.Symbol.Asks[i].Price);
    aTableItem := Items[iQuoteIndex];
    if aTableItem <> nil then
    begin
      if i > FAccumulHoga then
      ...
      iNewMin := Min(iNewMin, iQuoteIndex);
      iNewMax := Max(iNewMax, iQuoteIndex);
      SetQuoteField(tcQuote, ptShort, FQuote.Symbol.Asks[i].Volume);
    end;

  end;
  {
    // ASK part
  for i := 0 to FQuote.Asks.Count - 1 do
  begin
    FQuoteSums[ptShort] := FQuoteSums[ptShort] + FQuote.Asks[i].Volume;
    ...
    begin
      ...
      iNewMin := Min(iNewMin, iQuoteIndex);
      iNewMax := Max(iNewMax, iQuoteIndex);
      SetQuoteField(tcQuote, ptShort, FQuote.Asks[i].Volume);
    end;
  end;
  }
  //-- 3. Get Total min/max
  ...
  if iNewMin <= iNewMax then
    ...
    begin
      ...
      {
      // 밑으로 내결 갈때
      if (iOldMin < FStartIndex) and (iOldMax > FStartIndex) and (iOldMax <= FEndIndex) then
      begin
        if iNewMin < FStartIndex then
          iScrollSteps := iNewMin - FStartIndex;
      end else
      // 위로 올라갈때
      if (iOldMax > FEndIndex) and (iOldMin < FEndIndex) and (iOldMin >= FStartIndex) then
      begin
        if iNewMax > FEndIndex then
          iScrollSteps := iNewMax - FEndIndex;
      end else
      ...
      // 전체 아래에 있을때
      if ( iMid < FStartIndex) then
      begin
//        if iNewMax < FStartIndex then
//          iScrollSteps := iNewMin - FStartIndex;
        if iMid < FStartIndex then
          iScrollSteps := iMid - FStartIndex;
      end else
      // 전체 위에 있을때
      if (iMid > FEndIndex) then
      begin
        if iMid > FEndIndex then
          iScrollSteps := iMid - FEndIndex;
//        if iNewMin > FEndIndex then
//          iScrollSteps := iNewMax - FEndIndex;
      end;
    end;
  ...
  if bDraw then
  begin
    // iScrollSteps > 0 --> 스크롤 올라감
    // iScrollSteps < 0 --> 스크롤 내려감
  ...
  //DrawHideOwnOrderUpdate( iTotMin, iTotMax, false );
...
var
  ...
  function GetPosType : TPositionType;
  begin
          ...
          otNormal:
            begin
              ...
              begin // new order
                ...
                if QTIndex >= 0 then
                begin
                  Items[QTIndex].QTQtys[GetPosType] :=
                         Max(0,
                             Items[QTIndex].Quotes[GetPosType] -
                             aOrder.ActiveQty);
                  Items[QTIndex].QTRefCounts[GetPosType] := Items[QTIndex].QTRefCounts[GetPosType] + 1;
                  QTState := qtActive;

                  if Items[QTIndex].QTQtys[GetPosType] = 0 then
                  begin
                    if (QTIndex < FQuoteIndexLow) or (QTIndex > FQuoteIndexHigh) then
                      Items[QTIndex].QTQtys[GetPosType] := -1; // out of quote

                  end;

                  DrawQT(QTIndex, GetPosType);
                end else
                  QTState := qtDisabled;
              end;
              // -- QT END
...
//              // -- QT END
          ...
          otCancel :
            begin
              PositionType := GetPosType;//aOrder.Target.PositionType;
              GraphicType := ogCancel;
              QtyActionType := oqSub;
              FromIndex := FindIndex(aOrder.Price);
              ToIndex := FromIndex;
...
//        if aOrder.BetweenOrder then
//        begin
//          aTableItem := Items[FromIndex];
//          if aTableItem <> nil then
//            aTableItem.BetweenOrder := aOrder.BetweenOrder;
//        end else
//        begin
//          aTableItem := Items[FromIndex];
//          if aTableItem <> nil then
...
//        end;
    ...
    begin
      ...
      if aDrawItem.QTState = qtActive  then
        ...
        begin
          Items[aDrawItem.QTIndex].QTRefCounts[aDrawItem.PositionType] := Items[aDrawItem.QTIndex].QTRefCounts[aDrawItem.PositionType] -1;
          ...
          DrawRefresh( GetPosType, aTableItem, aDrawItem.QTIndex );
        end;
      // -- QT END
      aDrawItem.Free;
                ...
                begin
                  Items[QTIndex].QTQtys[aOrder.PositionType] :=
                         Max(0,
                             ...
                             aOrder.ActiveQty);
                  Items[QTIndex].QTRefCounts[aOrder.PositionType] := Items[QTIndex].QTRefCounts[aOrder.PositionType] + 1;
                  QTState := qtActive;
...
begin
  ...
  dPrice := 0;
  case aType of
    ptLong: dPrice := FQuote.Symbol.DayLow;
    ptShort:dPrice := FQuote.Symbol.DayHigh;
  end;
...
// 미결제 약정수량
procedure TOrderTablet.SetOpenPosition(dValue: double);
var
  iLeft, iTop, iRight, iBottom : Integer ;
  stText : String ;
  aBkColor, aFontColor : TColor ;
...
begin
  ...
  iTop := 0 ;
  iBottom := DEFAULT_HEIGHT ;
  stText :=  FloatToSTr(FOpenPosition) ;
  //
  if FOpenPosition > 0 then  //
  begin
    aBkColor := clRed; //SHORT_BG_COLOR ;
    aFontColor := clWhite; //clRed
  end
  else if FOpenPosition < 0 then
  begin
    aBkColor := clBlue; //LONG_BG_COLOR ;
...
//  aTickValue := LocksFromPrice( Symbol, price, 0 );
  aTickValue := UnitFromPrice(Symbol, price);
    //
...
function TOrderTablet.GetStopSum(aType: TPositionType): double;
begin
