...
type
  ...
  private
    ...
    procedure DrawTicks;
    procedure DrawAccumulTicks( bTick : boolean = true );
    procedure DrawAccumulTicksBack;
...
begin
  ...
  FTns  := FQuote.Symbol.Sales[FSelectedIndex];
...
begin
  ...
  dCur  := FDepth.Volume - FQuote.Symbol.Asks[0].Volume;
  if dCur = 0 then Exit;
...
procedure TTickPainter.DrawAccumulTicks( bTick : boolean );
var
  rRect : TRect ;
  aSale : TTimeNSale;
begin
  ...
  begin
    Brush.Color := clWhite;
    FillRect( Rect( FLefts[0] +1, 5 * FRowHeight+1, FRights[2], 6 * FRowHeight ) );
    Brush.Color := clBlue;
    FillRect( GetFillRect );

    if bTick then begin
      aSale := FQuote.Symbol.Sales[0];
      if (aSale <> nil) and ( aSale.Price = FDepth.Price ) then begin
        FVolume := FVolume + aSale.Volume ;
        FAccumulVolume := FAccumulVolume + aSale.Volume ;
      end;
    end;

    DrawRect(FBitmap.Canvas, Rect(FLefts[1]+1, 0 * FRowHeight+1 , FRights[2],
         (0 * FRowHeight) +FRowHeight),
             Format('%.*n',[ FQuote.Symbol.Spec.Precision, FDepth.Price]), clWhite, clBlack, taRightJustify);

    DrawRect(FBitmap.Canvas, Rect(FLefts[2]+1, 1 * FRowHeight+1 , FRights[2],
         (1 * FRowHeight) +FRowHeight),
             Format('%.0n',[ FVolume * 1.0 ]), clWhite, clBlack, taRightJustify);

    DrawRect(FBitmap.Canvas, Rect(FLefts[1]+1, 2 * FRowHeight+1 , FRights[2],
         (2 * FRowHeight) +FRowHeight),
             Format('%.0n',[ FAccumulVolume * 1.0 ]), clWhite, clBlack, taRightJustify);

    DrawRect(FBitmap.Canvas, Rect(FLefts[2]+1, 3 * FRowHeight+1 , FRights[2],
         (3 * FRowHeight) +FRowHeight),
             Format('%.0n',[ FDepth.Volume * 1.0] ), clWhite, clBlack, taRightJustify);
...
procedure TTickPainter.DrawAccumulTicksBack;
const
  COL_TITLES : array[0..3] of String = ('호가','체결','누적체결','잔량');
var
  iRowCount, iColSpan, iColSpan2, i, iTop : Integer;
begin
  if not FShowFillBar then Exit;
  ...
  begin
    ...
    MoveTo(FLefts[0], 5 * FRowHeight );
    LineTO(FLefts[0], iColSpan2);

    MoveTo(FRights[2], 5 * FRowHeight );
    LineTO(FRights[2], iColSpan2);

    for i:=0 to 3-1 do
    begin
      if i=1 then continue;
      MoveTo(FRights[i], 0);
      LineTo(FRights[i], iColSpan);
    end;

    // titles
    for i:=0 to 3 do
      DrawRect(FBitmap.Canvas, Rect(FLefts[0]+1, i * FRowHeight+1 , FRights[0],
           (i * FRowHeight) +FRowHeight),
               COL_TITLES[i], $00EEEEEE, clBlack, taLeftJustify);
  end;
end;
...
begin
  ...
  if (FQuote = nil) or (FQuote.Symbol = nil) or (FQuote.Symbol.Spec = nil) then Exit;
  if FQuote.Symbol.Sales.Count < 1 then Exit;
  aSale := FQuote.Symbol.Sales[0];
  if ( FUseVolumeFilter ) and (FVolumeThreshold > aSale.Volume) then Exit;
  ...
  DrawRect(FBitmap.Canvas, Rect(FLefts[1]+1, iTop+1, FRights[1], iBot),
           FQuote.Symbol.PriceToStr(aSale.Price),
  ...
  DrawRect(FBitmap.Canvas, Rect(FLefts[2]+1, iTop+1, FRights[2], iBot),
           Format('%.*n', [ GetQtyPrecision(FQuote.Symbol, aSale.Price), aSale.Volume*1.0]),
