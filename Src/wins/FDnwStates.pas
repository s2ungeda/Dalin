...
uses
  ...
  Vcl.ComCtrls
  , UStorage, USymbols
  , UApiTypes, UDistributor, Vcl.Mask  , UTypes
  ;
...
begin
  ...
  case iCol of
    ...
    12 : Result := aSymbol.PriceToStr( aSymbol.Last );
    14 : Result := Format('%.*n', [ 0, aSymbol.DayAmount ]);
  end;
end;
...
begin
  ...
  case iCol of
    9 : Result := aSymbol.Basis;
    10 : Result := aSymbol.KimpPrice;
    11 : REsult := aSymbol.WDCPrice;
    13 : if aSymbol.PrevClose <= 0 then
           Result := 0.0
         else
...
begin
  ...
  with sgDnw do
    ...
    begin
      ...
      Cells[iCol, i] := GetData(aSymbol[2], iCol, i = iRow);       inc(iCol);       // iCol = 8
      inc(iCol);     j := ifThen( i = iRow, 1, 2 );
      //basis
      bSymbol := App.Engine.SymbolCore.BaseSymbols.FindSymbol( aSymbol[0].Spec.BaseCode, ekBinance, eaFutUsdt  );
      if bSymbol <> nil then
        dBasis := bSymbol.Basis
      else
        dBasis := 0.0;
      Cells[iCol, i] := Format('%.*n %%', [ 2,  dBasis ]);       inc(iCol);
      //
      Cells[iCol, i] := Format('%.*n %%', [ FPrecision,  GetPriceData(aSymbol[j], iCol, i = iRow) ]);       inc(iCol);
      Cells[iCol, i] := Format('%.2f',    [ GetPriceData(aSymbol[j], iCol, i = iRow) ]);       inc(iCol);
...
begin
  ...
  begin
    ...
    begin
      ...
      if ACol in [1..7] then
        aBack := LONG_COLOR
      else if ACol in [10..11] then
        aBack := SHORT_COLOR;//GRID_MOD_COLOR;

      if ACol in [9..14] then
        dFormat := DT_RIGHT;
