  ...
  with sgVal do
    ...
    begin
      ...
      if i > 0  then  begin
        ...
        Cells[i, TimeRow ] := Format('%d:%s', [ (i-1) div 2 + 12, s ]);
...
begin
  ...
  with sgVal, App.Engine.SymbolCore do
    ...
    begin
      ...
      Cells[iCol,iRow+2]:= FmtString(FPrecision, JungKopi[ekBithumb][i] ) ;
      if i = 0  then
      begin
        SetColor( 0, sgVal, iCol, iRow );
        SetColor( 0, sgVal, iCol, iRow + 2 );
      end else
      begin
        SetColor( JungKopi[ekUpbit][i] - dPrev[ekUpbit], sgVal, iCol, iRow );
        SetColor( JungKopi[ekBithumb][i] - dPrev[ekBithumb], sgVal, iCol, iRow+2 );
      end;
...
begin
  ...
  begin
    ...
    begin
      aBack := clBtnFace;
      if (ACol = FCol) and ((( FRow-1 ) = ARow) or ((FRow+3)=ARow)) then
      begin
        aBack := clBlack;
        aFont := clWhite;
      end;
...
begin
  ...
  FData2             := App.Engine.ApiManager.ExRate.Value;

  if IsZero(  FData[ ekUpbit ] )       or IsZero( FData[ ekBithumb ] ) then Exit;
  with sgVal do
  begin
    Cells[0, 4] := FmtString( FPrecision, FData[ekUpbit]);
    Cells[0, 5] := FmtString( 1, FData2);
    Cells[0, 6] := FmtString( FPrecision, FData[ekBithumb]);
    ...
    Cells[FCol, FRow+1] := FmtString( 1, FData2);
    Cells[FCol, FRow+2] := FmtString( FPrecision, FData[ekBithumb]);

    if ( FCol = 1 ) and ( FRow = 1 ) then
    begin
      SetColor( 0, sgVal, iCol, FRow );
      SetColor( 0, sgVal, iCol, FRow + 2 );
    end   ;

    idx := Max( 0, iCol -1 );
    SetColor( FData[ ekUpbit ]    - App.Engine.SymbolCore.JungKopi[ekUpbit][idx],  sgVal, FCol, FRow );
    SetColor( FData[ ekBithumb ]  - App.Engine.SymbolCore.JungKopi[ekBithumb][idx],  sgVal, FCol, FRow+2 );

    Objects[0,4] := Objects[ FCol, FRow];
    Objects[0,6] := Objects[ FCol, FRow+2];
