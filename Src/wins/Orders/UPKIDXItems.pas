...
type
  ...
  TPKIDXItem = class(TCollectionItem)
  private
    FLiqOrds: TOrderList;
    FOrders: TOrderList;
  ...
  TPKIDXItems = class(TCollection)
  private
    FDustQty: double;
    function GetPutKipItem(i:integer): TPKIDXItem;
  public
    Constructor Create;
    Destructor  Destroy; override;

    function New(iCnt : integer) : TPKIDXItem;

    procedure SetDust(dDust : double);

    property DustQty : double read FDustQty;
    property PutKipItem[i:integer] : TPKIDXItem read GetPutKipItem;
  end;
...
procedure TPKIDXItem.CalcCustomIdx(const dPrice: double);
begin
  FIdxPk := FKip - ((dPrice / App.Engine.ApiManager.ExRate.Value * 100) - 100);
end;

procedure TPKIDXItem.CalcFillAvgKip;
var
  I: Integer;
  aOrder : TOrder;
...
procedure TPKIDXItem.CalcIdxPk(const dPrice: double);
begin
  FTPrice := dPrice;
...
procedure TPKIDXItem.CheckLiqOrders;
var
  i : integer;
begin
  i := 0;
...
procedure TPKIDXItem.DeleteLiqOrder(aOrder: TOrder);
var
  I: Integer;
begin
...
constructor TPKIDXItem.Create(aColl: TCollection);
begin
...
destructor TPKIDXItem.Destroy;
begin
  FLiqOrds.Free;
  inherited;
end;

procedure TPKIDXItem.Init;
begin
  FOrder := nil;
  FLiqOrds.Clear;
end;

function TPKIDXItem.IsChange: boolean;
begin
  Result := false;
...
function TPKIDXItem.IsDone: boolean;
var
  I: Integer;
  dFilledQty : double;
begin
  ...
  if FOrder = nil then Exit;
//
  // FOrder 주문수량 4,000 개 일때
  // 1. FOrder  4,000개 전량 체결 = 청산주문 4,000 개 체결 -> 해피 케이스
  // 2. FOrder  2,000개 일부 체결 나머지 취소 = 청산 주문 2,000 개 체결 -> so so
  //    --> 진입/청산 주문의 체결 수량만 같으면 Stop 했었음.
  //    --> 이제는 취소된 진입 2,000 개 도 다시 주문을 내야한다.
...
procedure TPKIDXItem.SetDust(dDust: double);
begin
  FDust := FDust + dDust;
end;

procedure TPKIDXItem.SetOrder(aOrder: TOrder; bLiq: boolean);
begin
  if bLiq then
    FLiqOrds.Add(aOrder)
  else
...
constructor TPKIDXItems.Create;
begin
  inherited Create(TPKIDXItem);
  FDustQty  := 0;
end;

destructor TPKIDXItems.Destroy;
begin

  inherited;
end;

function TPKIDXItems.GetPutKipItem(i:integer): TPKIDXItem;
begin
  if (i < 0) or (i >= Count) then
    Result := nil
  else
    Result := Items[i] as TPKIDXItem;
end;

function TPKIDXItems.New(iCnt: integer): TPKIDXItem;
begin
  Result := Add as TPKIDXItem;
  REsult.FIdx := iCnt;
end;

procedure TPKIDXItems.SetDust(dDust: double);
begin
