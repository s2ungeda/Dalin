...
//    if aSymbol = nil then Continue;
    if j = ekBinance then
      aSymbol := App.Engine.SymbolCore.BaseSymbols.FindSymbol(sCode, j, eaFutUsdt, scUSDT)
    else
      aSymbol := App.Engine.SymbolCore.BaseSymbols.FindSymbol(sCode, j, eaSpot, scKRW) ;
...
begin
  with sgKimp do
    case iCol of
      ...
      3 : Cells[ iCol, iRow] := Format('%.*n', [ 2, aSymbol.WDCPrice ]);
...
begin
  ...
  begin
    ...
    begin
      ...
      Cells[ 2, iRow] := Format('%.*n %%', [ 2, aSymbol.Basis ]);
...
//      Cells[ CurCol - 2, iRow] := ifThenStr( aSymbol.IsMargin, '○', 'X');
      ...
      Cells[CurCol-1, iRow]   := aSymbol.PriceToStr( aSymbol.IndexPrice );
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (ACol = 2 ) or ( ACol = 7 ) then
          //aBack :=  FUND_FORM_COLOR;
          aFont := clTeal;
      end;
      ...
      begin
        ...
        if Objects[ExCol, ARow] <> nil then
          ...
          if (ACol = 2 )or ( ACol = CurCol-1) then
            dFormat := DT_RIGHT;
        // 매도/매수 하이라이트
