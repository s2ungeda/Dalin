...
begin
  ...
  App.Log(llInfo, 'TFormBroker Destroy' );
end;
...
begin
  ...
  if aForm = nil then
  begin
    App.Log(llError, '', 'Closed Form 못찾음');
    exit;
  end;
...
//{$IFDEF RELEASE}
  if aItem.FFormID = ID_SYSTEM_TRD then
  begin
    // 개별 화면 종료시..
    if App.AppStatus <> asClose then
    begin

      if not (MessageDlgPos('종료 하시겠습니까?', mtConfirmation, [mbYes,mbNo],
        0, aItem.FForm.Left + (aItem.FForm.Width div 2), aItem.FForm.Top) = mrYes) then
      begin
        Action := caNone;
        Exit;
      end;
...
begin
  ...
  //  주문설정은 하나만 띄운다.
  if (iFormID = ID_COMM_CONFIG) or (iFormID = ID_AUTO_ORD_CONFIG) then begin
...
begin
  ...
  begin
    ...
    //aItem := FindClassName((Application.MainForm.Components[i] as TForm).ClassName);
    aItem := FindForm(Application.MainForm.Components[i] as TForm);
    if aItem = nil then Continue;
