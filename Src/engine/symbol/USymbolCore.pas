...
type
  ...
  TFutureTypeArray = array [TExchangeKind,TFuturesType] of TSymbolList;
  ...
  private
    ...
    FMainExApiType: TExchangeApiType;
    FMainWDC: TMainWDCArray;

    FWCDays: TWCDDataList;
    FWCD30s: TWCDDataList;
    FFutureType: TFutureTypeArray;
    FTethers: TMajorSymbols;
  ...
  public
    ...
    function  RegisterSymbol(aExKind : TExchangeKind; aCode : string) : TSymbol ; overload;
...
//    function  FindQuoteSymbol(aExKind : TExchangeKind; sBaseCode : string ): TSymbol;
    ...
    // 국내 거래소 기준으로 계산
    function  CalcMinOrderQty(kSymbol, oSymbol: TSymbol; dPrice : double; iSide: integer) : double;
    ...
    procedure CalcUSDTKimp(aSymbol : TSymbol);
    function  CalcKip(aDomes, aOS : TSymbol; iFS: Integer; aoType: TAutoOrderType; dQty: double = 0): double;
    ...
    procedure CalcSP(aPrice : double; aSymbol: TSymbol ); overload;
    procedure CalcSP(mSymbol , sSymbol : TSymbol ); overload;
    ...
    procedure TopAmtSubscribe( aExKind : TExchangeKind; aSymbol1, aSymbol2 : TSymbol );
    ...
    property FutureType : TFutureTypeArray read FFutureType;
    ...
    property Tethers     : TMajorSymbols read FTethers;

    property MainKimp : TMainKimpArray read FMainKimp ;//write FMainKimp;
    property MainWDC  : TMainWDCArray  read FMainWDC;

    property MainExApiType : TExchangeApiType read FMainExApiType write FMainExApiType;
...
function UnitFromPrice(aSymbol: TSymbol; dPrice: Double): double;
function GetPrecision(aSymbol: TSymbol; dPrice: Double): integer;
function GetQtyPrecision(aSymbol: TSymbol; dPrice: Double): integer;
...
// 주거래소, 서브거래소
procedure TSymbolCore.CalcSP(mSymbol, sSymbol: TSymbol);
begin
//  // 진입SP;
//  if IsZero(mSymbol.Bids[0].Price) then
//    mSymbol.SPValue[0]  := 0
//  else
//    mSymbol.SPValue[0] :=
//    ( sSymbol.Bids[0].Price - mSymbol.Bids[0].Price ) / mSymbol.Bids[0].Price * 100;
//
//  // 청산SP
//  if IsZero(mSymbol.Asks[0].Price) then
//    mSymbol.SPValue[1]  := 0
//  else
//    mSymbol.SPValue[1]  :=
//    ( sSymbol.Asks[0].Price - mSymbol.Asks[0].Price ) / mSymbol.Asks[0].Price * 100;

//  // 진입SP;
//  if IsZero(mSymbol.Bids[0].Price) then
//    mSymbol.SPValue[0]  := 0
//  else
//    mSymbol.SPValue[0] := ( sSymbol.Bids[0].Price / mSymbol.Bids[0].Price - 1 ) * 100;
//
//  // 청산SP
//  if IsZero(mSymbol.Asks[0].Price) then
//    mSymbol.SPValue[1]  := 0
//  else
//    mSymbol.SPValue[1]  := ( sSymbol.Asks[0].Price / mSymbol.Asks[0].Price -1 ) * 100;


  // 마지막 버전
  // 진입SP;
// 시세 모니터링
//[오후 4:35] SP 진입 = (빗썸현재가 - 업비트 매수호가) / 업비트 매수호가
//[오후 4:35] SP 청산 = (빗썸현재가 - 업비트 매도호가) / 업비트 매도호가


// SP 주문.
//[오후 4:35] SP 진입 = (빗썸매수호가 - 업비트 매수호가) / 업비트 매수호가
//[오후 4:35] SP 청산 = (빗썸매도호가 - 업비트 매도호가) / 업비트 매도호가

  if IsZero(mSymbol.Bids[0].Price) then begin
    mSymbol.SPValue[0]  := 0;
    mSymbol.TrdSpValue[0]:= 0;
  end
  else begin
    mSymbol.SPValue[0] := ( sSymbol.Last - mSymbol.Bids[0].Price ) / mSymbol.Bids[0].Price * 100;
    mSymbol.TrdSpValue[0] := ( sSymbol.Bids[0].Price - mSymbol.Bids[0].Price ) / mSymbol.Bids[0].Price * 100;
  end;

  // 청산SP
  if IsZero(mSymbol.Asks[0].Price) then begin
    mSymbol.SPValue[1]  := 0;
    mSymbol.TrdSpValue[1] := 0;
  end
  else begin
    mSymbol.SPValue[1] :=  ( sSymbol.Last - mSymbol.Asks[0].Price ) / mSymbol.Asks[0].Price * 100;
    mSymbol.TrdSpValue[1] :=  ( sSymbol.Asks[0].Price  - mSymbol.Asks[0].Price ) / mSymbol.Asks[0].Price * 100;
  end;


end;

procedure TSymbolCore.CalcUSDTKimp(aSymbol: TSymbol);
begin
  //aSymbol.KimpPrice := CalcDiv(aSymbol.Last - App.Engine.ApiManager.ExRate.Value, aSymbol.Last) * 100;
  aSymbol.KimpPrice := CalcDiv(aSymbol.Last, App.Engine.ApiManager.ExRate.Value) * 100 - 100;
end;

procedure TSymbolCore.CalcIndex(aSymbol: TSymbol);
begin
  if (aSymbol.Code = 'USDT') then
    CalcUSDTKimp(aSymbol);

  if App.Config.TradeMod then Exit;
  CalcKimp( aSymbol );
...
begin
  ...
  begin
    ...
    if aSymbol.Spec.ExApiType <> FMainExApiType then Exit;
    ...
    begin
      ...
      begin
         ...
         // 선물 현재가 대신 인덱스 가격(현물가격) 사용
         CalcKimp( aSymbol.IndexPrice, pSymbol );
         // 계산된 kip 에 basis 를 더해준다.
         // basis 더하지 말랜다.
         //pSymbol.KimpPrice := pSymbol.KimpPrice + aSymbol.Basis;
      end;
    end;

  end else
  begin
    pSymbol := FBaseSymbols.FindSymbol( aSymbol.Spec.BaseCode, FMainExKind, FMainExApiType  );
    if pSymbol <> nil then begin
      //CalcKimp( pSymbol.Last, aSymbol );
      CalcKimp( pSymbol.IndexPrice, aSymbol );
      // 계산된 kip 에 basis 를 더해준다.
      // basis 더하지 말랜다.
      //aSymbol.KimpPrice := aSymbol.KimpPrice + pSymbol.Basis;
    end;
    //  빗썸 USDT 김프는 다른방식으로 구함.
    if (aSymbol.Code = 'USDT') {and (aSymbol.Spec.ExchangeType = ekBithumb)} then
      CalcUSDTKimp(aSymbol);
...
function TSymbolCore.CalcKip(aDomes, aOS: TSymbol; iFS: Integer; aoType: TAutoOrderType;
   dQty: double): double;
var
  dEstPrice, dEx, dMo, dJa : double;
  dTmp: double;
begin
  Result := 0 ;

  dEx := App.Engine.ApiManager.ExRate.GetExRate;

  if (aDomes = nil) or (aOS = nil) then Exit;
  ...
  case aoType of
    aoEntry:
      case iFS of
        // F
        0 : begin
              dEstPrice := aDomes.CalcEstPrice(1, dQty);
              dJa       := dEstPrice;
              dMo       := aOs.Asks[0].Price * dEx;
          end;
        // S
        1 : begin
              dEstPrice := aOS.CalcEstPrice(-1, dQty);
              dJa       := aDomes.Bids[0].Price;
              dMO       := dEstPrice * dEx;
          end;
      end;
    aoExit:
      case iFS of
        0 : begin
              dEstPrice := aDomes.CalcEstPrice(-1, dQty);
              dJa       := dEstPrice;
              dMo       := aOs.Bids[0].Price * dEx;
          end ;
        1 : begin
              dEstPrice := aOS.CalcEstPrice(1, dQty);
              dJa       := aDomes.Asks[0].Price;
              dMo       := dEstPrice * dEx;
          end;
      end;
  end;

  if dMo <= EPSILON then Exit;

  Result := (dJa / dMo -1) * 100;

//  if (IFS = 1) and (aoType = aoEntry) then
//  begin
//     App.Log(llDebug, 'eeee','symbolcore : %s, %s, %s, %s', [ dQty.ToString, dEstPrice.ToString, dJa.ToString, Result.ToString] );
//  end;
...
function TSymbolCore.CalcMinOrderQty(kSymbol, oSymbol: TSymbol; dPrice: double;
  iSide: integer): double;
  var
    iPrecision : integer;
    dMinQty, dMinAmt, oPrice    : double;
begin
  Result := 0;
  if (kSymbol = nil) or (oSymbol = nil) then Exit;
  if (iSide > 0) and (oSymbol.Asks[0].Price <= EPSILON) then Exit
       else oPrice := oSymbol.Asks[0].Price;

  if (iSide < 0) and (oSymbol.Bids[0].Price <= EPSILON) then Exit
       else oPrice := oSymbol.Bids[0].Price;

  iPrecision := Min(kSymbol.Spec.QtyPrecision, oSymbol.Spec.QtyPrecision);
  dMinQty    := Max(kSymbol.Spec.MinNotional / dPrice,   oSymbol.Spec.MinNotional / oPrice);

  Result := CeilDecimal(dMinQty, iPrecision);
end;
...
begin
  ...
  begin
    ...
    for j := ftUsdt to High(TFuturesType) do
      FFutureType[i][j] := TSymbolList.Create;
  end;
...
begin
  ...
  inherited;
   ...
   App.Log(llInfo, 'TSymbolCore Destroy' );
end;
...
begin
  if ( aSymbol.Spec.ExchangeType = FMainExKind )
    and ( aSymbol.Spec.ExApiType = FMainExApiType ) then
    Result := true
  else
    Result := false;
end;
...
begin
  ...
  begin
    App.DebugLog('======  %03d : BASE : %s ( %d ) ========', [ j, FBaseSymbols[j], TList<TSymbol>(FBaseSymbols.Objects[j]).Count ]  )    ;
    with TList<TSymbol>(FBaseSymbols.Objects[j]) do
      for k := 0 to Count-1 do
      begin
        aSymbol := Items[k];// Symbols[k];

        App.DebugLog('%03d(%03d) : %s, %s %s %s', [ j, k, aSymbol.Code,
          TExchangeKindDesc[aSymbol.Spec.ExchangeType],
          TExApiTypeDesc[ aSymbol.Spec.ExApiType], aSymbol.Spec.SettleCode]);
      end;
...
begin
  ...
  for j := ekBinance to High(TExchangeKind) do
    if FTethers[j] <> nil then
      App.Engine.QuoteBroker.Brokers[j].Subscribe(Self,  FTethers[j],
        App.Engine.QuoteBroker.Brokers[j].DummyEventHandler
        );
  ...
  case aMarket of
    ...
    mtFutures:
      begin
        //
        sUnder  := aCode;
        stSuf   := Copy(aCode, 1, 4 );
        if stSuf = '1000' then
          sUnder := Copy(aCode, 5, Length(aCode));

        aUnder := FSpots[aExKind].Find( sUnder );
        if aUnder = nil then
          Exit ( nil );
...
// 개발 당시 바이낸스 COIN-M 선물만 이쪽으로 옴..
function TSymbolCore.RegisterSymbol(aExKind: TExchangeKind; aCode: string): TSymbol;
  var
    aSpec : TMarketSpec;
    sFQN  : string;
    aUnder: TSymbol;
...
begin
  ...
  aUnder := FSpots[aExKind].Find( sts[0]+'T' );
  if aUnder = nil then
    Exit ( nil );
  ...
  sFQN   := Format( '%s@%s.%s', [ aCode, sts[0], GetBaseFuturesFQN( aExKind ) ]);
  aSpec  := FSpecs.Find( sFQN );
  if aSpec = nil then
  begin
    aSpec := FSpecs.New(sFQN);
...
begin
  ...
  begin
      ...
      begin
        aSymbol := nil;
        if j= ekBinance then
          aSymbol := FBaseSymbols.FindSymbol( TMajorSymbolCode[i], j, FMainExApiType)
        else
...
procedure TSymbolCore.TopAmtSubscribe(aExKind: TExchangeKind; aSymbol1,
  aSymbol2: TSymbol);
  var
    aSymbol : TSymbol;
    I: Integer;
begin
  if (aSymbol1 = nil) or ( aSymbol2 = nil) then Exit;

  for I := 0 to 1 do
  begin
    aSymbol := App.Engine.ApiManager.ExManagers[aExKind].TopAmtSymbols[i] ;
    if aSymbol <> nil then
    begin
      if (aSymbol <> aSymbol1) and (aSymbol <> aSymbol2) then
        App.Engine.QuoteBroker.Brokers[aExKind].Cancel(Self,aSymbol);
    end;
  end;

  if (aSymbol1 <> App.Engine.ApiManager.ExManagers[aExKind].TopAmtSymbols[0])
    and (aSymbol1 <> App.Engine.ApiManager.ExManagers[aExKind].TopAmtSymbols[1])  then
      App.Engine.QuoteBroker.Brokers[aExKind].Subscribe(Self, aSymbol1,
        App.Engine.QuoteBroker.Brokers[aExKind].DummyEventHandler );

  if (aSymbol2 <> App.Engine.ApiManager.ExManagers[aExKind].TopAmtSymbols[0])
    and (aSymbol2 <> App.Engine.ApiManager.ExManagers[aExKind].TopAmtSymbols[1])  then
      App.Engine.QuoteBroker.Brokers[aExKind].Subscribe(Self, aSymbol2,
        App.Engine.QuoteBroker.Brokers[aExKind].DummyEventHandler );

  App.Engine.ApiManager.ExManagers[aExKind].TopAmtSymbols[0] := aSymbol1;
  App.Engine.ApiManager.ExManagers[aExKind].TopAmtSymbols[1] := aSymbol2;
...
begin
  ...
  begin
    ...
    begin
      ...
      if aSymbol.Spec.ExchangeType = ekBinance then begin
        aSymbol.ReSizeMarketDepth(20);
      end;

      FFutureType[aExKind][aSymbol.Spec.FutureType].AddSymbol(aSymbol);
    end;
    ...
    //if aSymbol.Code <> 'USDTUSDT' then
    FBaseSymbols.AddSymbol( aSymbol );

  end;

  if (aSymbol.Code <> 'USDTUSDT') and (aSymbol.Spec.BaseCode = 'USDT') then
    FTethers[aSymbol.Spec.ExchangeType] := aSymbol;
...
begin
  ...
  case aSymbol.Spec.ExchangeType of
    ekBinance: Result := dPrice + iTicks * aSymbol.Spec.TickSize;
    ekUpbit:
      begin
        ...
        begin
          if Result > 2000000.0 - EPSILON then
            ...
            Result := Result + 100
          else if Result > 50000 - EPSILON then   // 신설
            Result := Result + 50
            ...
            Result := Result + 1
          else if Result > 100 - EPSILON then   // 1000 이랑 차이가 없는데...
            Result  := Result + 1
            ...
            Result := Result + 0.001  //
          else if Result > 0.01 - EPSILON  then
            Result := Result + 0.0001  //
          else if Result > 0.001 - EPSILON  then
            Result := Result + 0.00001  //
          else if Result > 0.0001 - EPSILON then
            Result := Result + 0.000001
          else if Result > 0.00001 - EPSILON then
            Result := Result + 0.0000001
          else
            Result := Result + 0.00000001;
        end else
        begin
          if Result < 0.00001 + EPSILON then
            Result := Result - 0.00000001
          else if Result < 0.0001 + EPSILON then      //  0.00001 ~ 0.0001
            Result := Result - 0.0000001
          else if Result < 0.001 + EPSILON then       //  0.0001  ~ 0.001
            Result := Result - 0.000001
          else if Result < 0.01 + EPSILON then
            Result := Result - 0.00001
          else if Result < 0.1 + EPSILON then
            ...
            Result := Result - 1
          else if Result < 5000 + EPSILON then        //  1000   ~  5000
            Result := Result - 1
          else if Result < 10000 + EPSILON then       //  5000   ~  10000
            Result := Result - 5
...
begin
  case aSymbol.Spec.ExchangeType of
    ekBinance: Result := aSymbol.Spec.TickSize;
    ekUpbit:
      begin

        if dPrice > 2000000.0 - EPSILON then
          Result := 1000
        else if dPrice > 1000000 - EPSILON then
          Result := 1000
        else if dPrice > 500000 - EPSILON then
          Result := 500
        else if dPrice > 100000 - EPSILON then
          Result := 100
        else if dPrice > 50000 - EPSILON then   // 신설
          Result := 50
        else if dPrice > 10000 - EPSILON then
          Result := 10
        else if dPrice > 5000 - EPSILON then
          Result := 5
        else if dPrice > 1000 - EPSILON then
          Result := 1
        else if dPrice > 100 - EPSILON then   // 1000 이랑 차이가 없는데...
          Result := 1
          ...
          Result := 0.001  //
        else if dPrice > 0.01 - EPSILON  then
          Result := 0.0001  //
        else if dPrice > 0.001 - EPSILON  then
          Result := 0.00001  //
        else if dPrice > 0.0001 - EPSILON then
          Result := 0.000001
        else if dPrice > 0.00001 - EPSILON then
          Result := 0.0000001
        else
          Result := 0.00000001;
    ...
    ekBithumb:
      begin
        if dPrice > 1000000.0 - EPSILON then
          Result := 1000
        else if dPrice > 500000.0 - EPSILON then
          Result := 500
        else if dPrice > 100000 - EPSILON then
          Result := 100
...
begin
   case aSymbol.Spec.ExchangeType of
    ...
    ekUpbit:
      begin
        if dPrice > 100 - EPSILON then   // 1000 이랑 차이가 없는데...
          Result := 0
        else if dPrice > 10 - EPSILON then
          Result := 1
        else if dPrice > 1 - EPSILON then
...
begin
   case aSymbol.Spec.ExchangeType of
    ekBinance: Result := aSymbol.Spec.QtyPrecision  ;
    ekUpbit, ekBithumb:
      begin
        if dPrice > 1000000 - EPSILON then    // 100 만원 이상
          Result := 4
        else if dPrice > 100000 - EPSILON then  // 100만원 미만 10만원 이상
          Result := 3
        else if dPrice > 10000 - EPSILON then   // 10만원 미만 만원 이상
          Result := 2
        else if dPrice > 1000 - EPSILON then    // 만원 미만 천원 이상
          Result := 1
        else if dPrice > 1 - EPSILON then       // 천원 미만 1원 이상
          Result := 0
        else                                    // 1원 미만
          Result := -1;
      end;
