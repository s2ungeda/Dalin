...
begin
  ...
  if ( FSymbol.Sales <> nil ) and ((FLastEvent = qtTimeNSale) or (FLastEvent = qtCustom)) then
  begin
    ...
    if (FSymbol.Spec.ExchangeType <> ekBinance) and  (FSymbol.Code = 'USDT') then begin
      FSymbol.Terms.Update(FSymbol.GetCustomIndex);
    end;
  ...
  if dtTime > FSymbol.LastEventTime then
...
begin
  ...
  try
    // 1. 호가차 계산
    if (IsZero( FSymbol.Bids[0].Price )) or (IsZero( FSymbol.Asks[0].Price))   then
      FSymbol.SpreadRate := 0
    else begin
      dTmp := Max( (FSymbol.Asks[0].Price - FSymbol.Last), ( FSymbol.Last - FSymbol.Bids[0].Price));
      dTmp := Max( dTmp, (FSymbol.Asks[0].Price - FSymbol.Bids[0].Price ) );
      dRate:= dTmp/ FSymbol.Bids[0].Price * 100;
      FSymbol.SpreadRate := dRate;
    end;
...
procedure TQuote.CalcDelayMS;
begin

  FSymbol.DelayTime := Frac(  FSymbol.LastTime - FSymbol.LastEventTime );
  if FSymbol.DelayTime < FSymbol.MinDelayTime then
    FSymbol.MinDelayTime := FSymbol.DelayTime;

  FSymbol.DelayMs := Round((FSymbol.DelayTime-FSymbol.MinDelayTime) * 24 * 3600 * 1000);
  if FSymbol.DelayMs > 1000000 then
    FSymbol.DelayMs := 100;

  App.ExData[ FSymbol.Spec.ExchangeType ].SetDelayData( FSymbol.Spec.BaseCode, FSymbol.DelayMS / 1000 );
...
begin
  ...
  App.Log(llInfo, 'TQuoteBrokerManager Destroy' );
end;
