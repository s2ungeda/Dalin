...
//    property FiveTerm : TSTermItem read GetTermItem;
  ...
  public
    ...
    procedure GetList(var aList: TList<TSymbol>; sCurrency : string;
      aExKind : TExchangeKind; aExApiType : TExchangeApiType);
...
begin
  ...
  if aSymbolList <> nil then
    ...
    begin
      aSymbol := aSymbolList[i];
      if (aSymbol.Spec.ExchangeType = aExKind) and (aSymbol.Spec.SettleType = aSCType)
         and ( aSymbol.Spec.ExApiType = aExApiType) then
      begin
        Result := aSymbol;
        Break;
...
procedure TBaseSymbols.GetList(var aList: TList<TSymbol>; sCurrency: string;
  aExKind: TExchangeKind; aExApiType: TExchangeApiType);
var
  aSymbolList : TList<TSymbol>;
  aSymbol : TSymbol;
...
begin
  ...
  aSymbolList := FindSymbolList(sCurrency) ;
  if aSymbolList <> nil then
    for I := 0 to aSymbolList.Count-1 do
    begin
      aSymbol := aSymbolList[i];

      if (aSymbol.Spec.ExchangeType = aExKind) and (aSymbol.Spec.ExApiType = aExApiType) then
        aList.Add(aSymbol);
    end;
end;
