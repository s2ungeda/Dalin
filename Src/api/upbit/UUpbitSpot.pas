...
//    procedure parseTicker;
  ...
  public
    ...
    function RequestMaster : boolean ; overload; override;
    ...
    function RequestBalance : boolean; overload; override;
...
//    procedure RequestAvailableAmt( aSymbol : TSymbol );
    ...
    function GetSig(aReq: TRESTRequest; sQuery : string ): string; overload;
...
//		procedure RequestBalance( aSymbol : TSymbol ) ; overload; override;
    ...
    property  MarketParam : string read FMarketParam;
      //
    property  LastSendTime: TDateTime read FLastSendTime;
  end;
...
constructor TUpbitSpot.Create(aObj: TObject; eaType : TExchangeApiType);
begin
...
procedure TUpbitSpot.RequestStuckOrderList(aList: TStringList);
var
  sJson, sReq, sSig, sOut: string;
  bRes : boolean;
  i:integer;
begin
  ...
  try
    ...
    for I := 0 to aList.Count-1 do
      if i = 0 then
        sReq := sReq + 'identifiers[]'
      else
        sReq := sReq + '&identifiers[]='+aList[i];  
        
    GetSig(sReq);   

    if Request( rmGET, '/v1/orders/uuids?'+sReq, '', sJson, sOut ) then
    begin
      gUpReceiver.ParseOrdersFromLNO(sJson);
    end else
    begin
      App.Log( llError, '', 'Failed %s RequestOrdersFromLNO (%s, %s)',
        [ TExchangeKindDesc[GetExKind], sOut, sJson] );
      Exit;
    end;
...
begin
  ...
  begin
    ...
    aReq.AMethod	:= rmGET;
    aReq.Req.init( App.Engine.ApiConfig.GetBaseUrl( GetExKind , eaSpot ) );
...
begin
  ...
  try
    ...
    LToken.Claims.SetClaimOfType<string>('access_key', App.Engine.ApiConfig.GetApiKey( GetExKind , eaSpot ));
    LToken.Claims.SetClaimOfType<string>('nonce', sID );

    sSig := TJOSE.SerializeCompact(  App.Engine.ApiConfig.GetSceretKey( GetExKind , eaSpot )
...
function TUpbitSpot.GetSig(aReq: TRESTRequest; sQuery: string): string;
var
  LToken: TJWT;
  guid : TGUID;
  sSig, sID, sToken : string;
...
function TUpbitSpot.RequestBalance: boolean;
begin
  Result := RequestAccounts;
end;
...
function TUpbitSpot.RequestOrders: boolean;
var
  sJson, sSig, sOut: string;
  bRes : boolean;
begin
	bRes := false;
  ...
  try
    ...
    if Request( rmGET, '/v1/orders/open', '', sJson, sOut ) then
    begin
      gUpReceiver.ParseActiveOrder(sJson);
    end else
    begin
...
function TUpbitSpot.RequestCandleData(aSymbol: TSymbol; sUnit: string; iDiv,
...
begin
  ...
  try
    ...
    sRrs := Format('v1/candles/%s/%d?market=%s&count=%d', [sTmp, iDiv, aSymbol.OrgCode, iCount] );
    if Request( rmGET, sRrs, '', sJson, sOut ) then
    begin
      gUpReceiver.ParseCandleData(aSymbol, sJson);
    end else
    begin
      App.Log( llError, '', 'Failed %s RequestCandleData (%s, %s)',
        [ TExchangeKindDesc[GetExKind], sOut, sJson] );
...
function TUpbitSpot.RequestWithdrawAddress: boolean;
var
  sJson, sOut, sSig: string;
begin
  Result := false;
  ...
  if Request( rmGET, '/v1/withdraws/coin_addresses', '', sJson, sOut ) then
  begin
    gUpReceiver.ParseWithdrawAddress(sJson);
  end else
  begin
...
begin
  ...
  // 빗썸이 로컬넘버 서비스 오픈 했기에 주문번호 -> 로컬번호
  // 접수가 누략이 되면, 주문번호를 알수 없기에
//  if sData <> '' then
  	App.Engine.SharedManager.RequestData( GetExKind, eaSpot,
        rtOrdDetail,  aOrder.OrderNo, aOrder.LocalNo
      )  ;
end;
...
begin
	App.Engine.SharedManager.RequestData( GetExKind, aSymbol.Spec.ExApiType,
...
begin
  ...
  begin
    ...
    Result := App.Engine.SharedManager.RequestData( GetExKind, aOrder.Symbol.Spec.ExApiType,
...
//    rtBalance   : gUpReceiver.ParseSpotBalance( aData, aRef ) ;          // aref is symbol code
    ...
    rtTradeAmt  : gUpReceiver.ParseSpotTradeAmt(aData, aRef);
  end;
