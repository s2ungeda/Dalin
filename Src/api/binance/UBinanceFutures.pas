...
uses
  ...
  UExchange, UBinanceWebsocketApi,
...
type
  ...
  public
    ...
    property WsApi : TBinanceWebsocketApi read FWsApi;
...
procedure TBinanceFutures.Connect;
begin
  FWsApi.DoConnect;
end;

constructor TBinanceFutures.Create(aObj: TObject; eaType : TExchangeApiType);
begin
  ...
  FWsApi := TBinanceWebsocketApi.Create(eaType);
end;
...
procedure TBinanceFutures.Disconnect;
begin
  FWsApi.DoDisConnect;
end;
...
begin
  ...
  sig  := CalculateHMACSHA256( data, App.Engine.ApiConfig.GetSceretKey( GetExKind, eaFutUsdt) );

  SetParam('timestamp', sTime );
  SetParam('signature', sig );
  SetParam('X-MBX-APIKEY', App.Engine.ApiConfig.GetApiKey( GetExKind, eaFutUsdt ), pkHTTPHEADER );
...
function TBinanceFutures.RequestChangeLeverage(stCode,
...
begin
  ...
  try
    ...
    if Request( rmPOST, '/fapi/v1/leverage', '', sJson, sOut ) then
    begin
  //    App.Log( llDebug, '', '%s (%s, %s)', [ TExchangeKindDesc[GetExKind], sOut, sJson] );
...
function TBinanceFutures.RequestChangeMarginType(stCode,
...
begin
  ...
  sig  := CalculateHMACSHA256( data, App.Engine.ApiConfig.GetSceretKey( ekBinance , eaFutUsdt ) );
  ...
  if Request( rmPOST, '/fapi/v1/marginType', '', sJson, sOut ) then
  begin
//    App.Log( llDebug, '', '%s (%s, %s)', [ TExchangeKindDesc[GetExKind], sOut, sJson] );
...
begin
  ...
  if bFirst then aMethod := rmPOST
    ...
    aMethod := rmPUT;
    App.DebugLog('------------ USD-M futures listenkey update --------------' );
  end;

  SetBaseUrl(    App.Engine.ApiConfig.GetBaseUrl( GetExKind , eaFutUsdt ) );
  SetParam('X-MBX-APIKEY', App.Engine.ApiConfig.GetApiKey( GetExKind , eaFutUsdt ), pkHTTPHEADER );
  ...
  begin
    App.Log( llError, '', 'Failed %s %s RequestListenKey (%s, %s)', [ TExchangeKindDesc[GetExKind], TMarketTypeDesc[mtFutures], sOut, sJson] );
    aObj  := RestRes.JSONValue as TJsonObject;
    if aObj <> nil then
    begin
      var ival : integer;
...
begin
  ...
  sSig  := CalculateHMACSHA256( sData, App.Engine.ApiConfig.GetSceretKey( GetExKind , eaFutUsdt ) );
...
// 바이낸스는 조회하지 않는다.
procedure TBinanceFutures.RequestOrderDetail(aOrder: TOrder);
begin
//  FWsApi.QueryOrder(aOrder);
end;
...
begin
  ...
  if Request( rmGET, '/fapi/v1/openOrders ', '', sJson, sOut ) then
  begin
    gBinReceiver.ParseActiveOrder(sJson);
  end else
  begin
    App.Log( llError, '', 'Failed %s %s RequestOrders (%s, %s)',
      [ TExchangeKindDesc[GetExKind], TExApiTypeDesc[eaFutUsdt], sOut, sJson] );
    Exit(false);
  end;
...
//  Exit;
	// '|' 를 구분자 사용
  Result := FWsApi.SenderOrder(aOrder);
