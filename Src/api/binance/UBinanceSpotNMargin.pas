...
uses
  ...
  , UFQN
  , UEncrypts, UDecimalHelper
  ;

{ TBinanceSpotNMargin }

constructor TBinanceSpotNMargin.Create(aObj: TObject; eaType : TExchangeApiType);
begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              bInt.convert(sTmp);
              iQtyPre := bInt.Precision;// GetPrecision( sTmp );
            end else
            if aFiType = 'NOTIONAL' then
            begin
              sTmp  := aFil.GetValue('minNotional').Value;
              bInt.convert(sTmp);
              dNotion := bInt.ToDouble;
            end else
            if aFiType = 'PERCENT_PRICE_BY_SIDE' then
            begin
              dUp     := aFil.GetValue<double>('bidMultiplierUp');
              dDown   := aFil.GetValue<double>('askMultiplierDown');
            end;
          end;
          ...
          if bNew then begin
            ...
            // USDT 가상 종목을 만든다.
            if aSymbol.Code = 'USDCUSDT' then
            begin
              sCode := 'USDTUSDT';
              var bSymbol := App.Engine.SymbolCore.RegisterSymbol(GetExKind, mtSpot, sCode);
              if bSymbol <> nil then
              begin
                bSymbol.TradeAble := false;
                with bSymbol do
                begin
                  OrgCode     := lowercase(sCode);
                  Spec.BaseCode    := 'USDT';
                  Spec.QuoteCode   := 'USDT';
//                  Spec.SettleCode  := 'USDT';
                end;
                App.Engine.SymbolCore.RegisterSymbol( GetExKind, bSymbol );
              end;
            end;
...
begin
  ...
  begin
    ...
    App.DebugLog('------------ spot listenkey update --------------' );
  end;

  SetParam('X-MBX-APIKEY', App.Engine.ApiConfig.GetApiKey( GetExKind , eaSpot ), pkHTTPHEADER );
  ...
  begin
    App.Log( llError, '', 'Failed %s %s RequestListenKey (%s, %s)',
      [ TExchangeKindDesc[GetExKind], TMarketTypeDesc[mtSpot],  sOut, sJson] );
    Exit;
  end;
end;
...
begin
  ...
  sig  := CalculateHMACSHA256( data, App.Engine.ApiConfig.GetSceretKey( GetExKind , eaSpot ) );
  ...
  if Request( rmGET, '/api/v3/account', '', sJson, sOut ) then
  begin
//    App.Log( llDebug, '', '%s (%s, %s)', [ TExchangeKindDesc[GetExKind], sOut, sJson] );
    gBinReceiver.ParseSpotBalance( sJson, '' );
  end else
  begin
    App.Log( llError, '', 'Failed %s RequestSpotBalance (%s, %s)',
      [ TExchangeKindDesc[GetExKind], sOut, sJson] );
    Exit(false);
  end;
...
function TBinanceSpotNMargin.RequestWithdrawAddress: boolean;
var
...
begin
  ...
  if Request( rmGET, '/sapi/v1/capital/withdraw/address/list', '', sJson, sOut ) then
    gBinReceiver.ParseWithdrawAddress(sJson, '' )
  else begin
...
//      rtBalance,  aSymbol.OrgCode , aSymbol.Spec.BaseCode
//      )  ;
//end;
...
begin
  ...
  if not App.Config.RunMod then Exit(true);
...
begin
  ...
  App.Log( llDebug, '', '%s : %s',  [ App.Engine.ApiConfig.GetSceretKey( GetExKind , eaSpot )
                                    , App.Engine.ApiConfig.GetApiKey( GetExKind , eaSpot )] );
...
begin
  ...
  if aReq.Seq = 0 then
  begin
    if not App.Config.RunMod then Exit;
    ...
    aReq.Req.AddParameter('X-MBX-APIKEY', App.Engine.ApiConfig.GetApiKey( GetExKind , eaSpot ), pkHTTPHEADER);
  end else
  begin
    // 주문낼때 다시 허용
...
begin
  ...
  with gBinReceiver do
	case aReqType of
    rtDnwState  : ParseDNWState( aData );
    rtBalance   : ParseSpotBalance(aData, aRef);
    rtNewOrder  : ParseSpotNewOrder(aData, aRef);
    rtCnlOrder  : ParseSpotCnlOrder(aData, aRef);
  end;
